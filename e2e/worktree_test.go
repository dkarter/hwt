...
func TestENV001_ENV002_ENV003_ENV004_EnvironmentStableRefreshAndRun(t *testing.T) {
	...
	if a["HWT_WORKTREE_HOSTNAME"] != "feature-env.localhost" || a["HWT_URL_WEB"] != "http://feature-env.web.localhost:"+a["HWT_PORT_WEB"].(string) || a["HWT_URL_ASSETS"] != "http://feature-env.assets.localhost:"+a["HWT_PORT_ASSETS"].(string) {
	...
	if refreshed["HWT_URL_WEB"] != "http://feature-env.web.localhost:"+refreshed["HWT_PORT_WEB"].(string) {
	...
	custom := decode(t, s.run(linked, "env", "--json"))["variables"].(map[string]any)
	if custom["HWT_URL_WEB"] != "https://app.acme.dev:"+custom["HWT_PORT_WEB"].(string) {
		t.Fatalf("custom service URL = %#v", custom)
	}
}
